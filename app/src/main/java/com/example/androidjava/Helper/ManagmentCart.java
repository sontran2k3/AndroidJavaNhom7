package com.example.androidjava.Helper;
...
public class ManagmentCart {
    ...
    public void insertFood(PopularDomain item){
        ...
        if(existAlready){
            listPop.get(n).setNumberrinCart(item.getNumberrinCart());
        }else{
            listPop.add(item);
        }
    ...
    public void minusNumberItem(ArrayList<PopularDomain> listItem, int position, ChangeNumberItemsListener changeNumberItemsListener){
        if(listItem.get(position).getNumberrinCart() == 1){
            listItem.remove(position);
        }else{
            listItem.get(position).setNumberrinCart(listItem.get(position).getNumberrinCart()-1);
        }
    ...
    public double getTotalFee(){
        ...
        for(int i = 0; i < listItem.size(); i ++ ){
            fee = fee+(listItem.get(i).getPrice() * listItem.get(i).getNumberrinCart());
        }
        return fee;
    }
}
